  ...
  function (needs) {
    ...
    needs.pretender((server, helper) => {
      server.get("/discourse-onboarding-banner/content.json", () => {
        return helper.response({ topic_id: 24, cooked: 'banner content goes here' });
      });
    ...
    test("DiscourseOnboardingBanner is shown and can be dismissed", async assert => {
      await visit("/");
      assert.ok(exists('.onboarding-banner-content'), "it shows the DiscourseOnboardingBanner");

      await click(".onboarding-banner-content .dismiss-banner");
      assert.ok(!exists('.onboarding-banner-content:visible'), "it hides the DiscourseOnboardingBanner");
    });
  }
);
